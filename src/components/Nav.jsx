...
function Nav() {
  return (
    <div className='bg-zinc-900 w-full'>
      <div className=" max-w-6xl mx-auto px-2 py-5 flex items-center justify-between border-b-[1px] border-zinc-700 ">
      

      <div className="navLeft flex items-center">
      <div className="logo flex items-center">
        <svg
          className="h-4 w-4 text-white bg-[transparent]"
          width="100%"
          height="100%"
          viewBox="0 0 15 19"
          fill="none"
        ...
        <h1 className="text-white font-semibold text-5 ml-2">Webora</h1>
      </div>
      <div className="tabs flex items-center mx-20">
        {["Home", "Work", "Culture", "", "News"].map((item, index) => 
        { return (item.length == 0) ? <span key={index} className='w-[1px] h-5 mx-10 inline-block bg-zinc-500'></span> : <a key={index} className=" text-white text-s mx-5 flex items-center gap-1" href="/blank"> {index == 1 && <span style={{boxShadow : "0 0 0.8em #0efc26"}} className=" inline-block rounded-full w-1.5 h-1.5 bg-green-500"></span>} {item}</a>}
